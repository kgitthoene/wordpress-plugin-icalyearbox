 ...
 *
 * @package WordPress Plugin YetAnotherICALCalendar/Includes
 */
class YetAnotherICALCalendar_Datespan {
  private $_from;
  private $_to;
  ...
  public function connects_to($other_span) {
    if (is_a($other_span, 'YetAnotherICALCalendar_Datespan')) {
      ...
      throw new ErrorException("Parameter must be type YetAnotherICALCalendar_Datespan!", 0, E_ERROR, __FILE__, __LINE__);
    }
  } // connects_to
  ...
  public function position($dt, $type = 'event') {
    if (is_a($dt, 'DateTime')) {
      if (($type == 'booking') or (($type == 'booking-split'))) {
        if (($dt == $this->_from) and ($dt == $this->_to)) {
          return YetAnotherICALCalendar_Datespans::IS_IN_SPAN;
        }
        if ($dt == $this->_to) {
          return YetAnotherICALCalendar_Datespans::IS_END;
        }
        if ($dt == $this->_from) {
          return YetAnotherICALCalendar_Datespans::IS_START;
        }
        if (($dt > $this->_from) and ($dt < $this->_to)) {
          return YetAnotherICALCalendar_Datespans::IS_IN_SPAN;
        }
    ...
    return YetAnotherICALCalendar_Datespans::IS_OUTSIDE;
  } // add
  ...
  public function inspect() {
    $value = sprintf("YetAnotherICALCalendar_Datespan(_from = '%s', _to = '%s', _description='%s')", $this->_from->format('c'), $this->_to->format('c'), $this->_description);
    return $value;
  }
} // class YetAnotherICALCalendar_Datespan


class YetAnotherICALCalendar_Datespans {
  ...
  public function add($span) {
    if (is_a($span, 'YetAnotherICALCalendar_Datespan')) {
  ...
  public function inspect() {
    $value = 'YetAnotherICALCalendar_Datespans(_spans = [';
